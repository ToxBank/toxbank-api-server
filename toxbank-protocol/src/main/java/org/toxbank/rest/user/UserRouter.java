...
public class UserRouter extends MyRouter {
	public UserRouter(Context context,ProtocolRouter protocols) {
		...
		attach(String.format("/{%s}%s",UserDBResource.resourceKey,Resources.project), ProjectDBResource.class);
		attach(String.format("/{%s}%s",UserDBResource.resourceKey,Resources.organisation), OrganisationDBResource.class);
